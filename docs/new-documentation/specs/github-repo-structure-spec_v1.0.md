...
## 2. The operation root repo — `Historiotheque/Historiotheque`

The cultural software itself, and the only repo that sees the whole operation:
official documents, specs, schemas, master bibliography, projects index.
(Note: for organizations, the profile-page README comes from a `.github` repo's
`profile/README.md` — the org-name repo does not render on the profile the way it
does for personal accounts. So the org's public face lives in `Historiotheque/.github`,
while this repo remains the operation's map and root.)
...
## 3. The universal Refcards repo — `Historiotheque/Refcards`

The atomic knowledge base. One concept per card, tens of thousands of them, on anything.
(The org namespace keeps it distinct from the older `antiface/Refcards` on the personal
account.)
...
2. The `Historiotheque/Refcards` repo with the format and the first cards (start with the named theories:
